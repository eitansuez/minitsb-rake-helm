...
print_command "export T1_GW_IP=\$(kubectl --context t1 get svc -n tier1 tier1-gateway --output jsonpath='{.status.loadBalancer.ingress[0].ip}')\n"

print_command "export C1_GW_IP=\$(kubectl --context c1 get svc -n bookinfo tsb-gateway-bookinfo --output jsonpath='{.status.loadBalancer.ingress[0].ip}')\n"

print_command "export C2_GW_IP=\$(kubectl --context c2 get svc -n bookinfo tsb-gateway-bookinfo --output jsonpath='{.status.loadBalancer.ingress[0].ip}')\n"
